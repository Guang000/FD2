...
from utils_recover import (inter_feature_center_dist, intra_feature_center_dist, features_dist, model_loss_append_cal,
                           draw_loss_cal)
...
def recover(task_info):
    ...
    if args.SC and num_call != args.ipc_start:
        for target in targets:
            ...
            for model_name, model, cal in zip(recover_model_name_list, recover_model_list, recover_cal_list):
                syned_intra_outputs = model(syned_intra_imgs)
                syned_intra_feature = last_feature_hooks[model_name].feature
                p_raw_syned, p_eff_syned, feature_matrix_syned, attention_map_syned, attention_maps_syned = cal(syned_intra_feature)
                syned_intra_attentions[model_name].append(attention_maps_syned)

    # initialization
    if args.initialisation_method == "Guassian":
        syning_imgs = torch.randn((targets.shape[0], 3, args.input_size, args.input_size), requires_grad=True,
                                  device=device, dtype=torch.float).to(device)
        print("initialisation method: Guassian")
    else: 
        syning_imgs = utils_re.initialize_patch_data(targets[0].item(), targets[-1].item()+1, args,
                                                     num_call, device).to(device)
        print(f"initialisation method: Patches--{args.patch_diff} ")
    lr, total_iteration = args.lr, args.iteration

    optimizer = optim.Adam([{'params': [syning_imgs], 'lr': lr}], betas=(0.5, 0.9), eps=1e-8)
    lr_scheduler = utils_re.lr_cosine_policy(lr, 0, total_iteration)
    criterion = nn.CrossEntropyLoss().to(device)

    start_time = time.time()
    # matplotlib Visualization
    save_every, iteration_plt, total_loss_plt, SC_loss_thresholds = 50, [], [], []
    model_CE_loss_cal, model_CE_loss_backbone, model_BN_loss, model_total_loss = {}, {}, {}, {}
    model_FC_loss, model_SC_loss = {}, {}
    for recover_model_name in recover_model_name_list:
        model_CE_loss_backbone[recover_model_name], model_CE_loss_cal[recover_model_name] = [], []
        model_total_loss[recover_model_name], model_BN_loss[recover_model_name] = [], []
        model_FC_loss[recover_model_name], model_SC_loss[recover_model_name] = [], []
    ...
    for iteration in range(total_iteration):
        ...
        optimizer.zero_grad()
        ce_lis_cal, ce_lis_backbone, fc_lis, SC_lis = [], [], [], []
        for model_name, model, cal, feature_center in zip(recover_model_name_list, recover_model_list, recover_cal_list, feature_centers):
            outputs_backbone = model(syning_imgs_jit)
            loss_ce_backbone = criterion(outputs_backbone, targets)
            last_feature = last_feature_hooks[model_name].feature
            p_raw, p_eff, feature_matrix, attention_map, attention_maps = cal(last_feature)
            loss_ce_cal = criterion(p_raw, targets)
            ce_lis_cal.append(loss_ce_cal)
            ce_lis_backbone.append(loss_ce_backbone)
            if args.FC:  
            ...
            if args.SC and num_call != args.ipc_start: 
                ...
                SC_lis.append(SC_loss_now_model)
        ...
        for id, BN_hook in enumerate(BN_hooks): 
            ...
            curr_loss_BN = curr_loss_BN_backbone if cal_ratio_list[id] == 0.0 else curr_loss_BN_backbone + curr_loss_BN_cal
            loss_BN_lis.append(curr_loss_BN)

        loss = torch.tensor(0.0, device=device)  
        for idx, weight in enumerate(weight_list):
            loss += weight * ( cal_ratio_list[idx] * ce_lis_cal[idx] +  (1. - cal_ratio_list[idx]) * ce_lis_backbone[idx]
            ...
            if args.SC and num_call != args.ipc_start:
                if SC_lis[idx].item() < args.SC_loss_threshold:
                    ...
                    loss = loss + weight * args.SC_ratio * SC_lis[idx]
        ...
        if iteration % save_every == 0 or iteration == total_iteration - 1: 
            ...
            for i, model_name in enumerate(recover_model_name_list):
                print(f"Model:{model_name}_cal,weight:{weight_list[i]}\n"
                      f"CE loss:{model_name}:{ce_lis_backbone[i].item()},"
            ...
            if args.matplotlib:
                ...
                (model_CE_loss_cal, model_CE_loss_backbone, model_BN_loss, model_total_loss, model_FC_loss,
                 model_SC_loss) \
                    = model_loss_append_cal(num_call, args, recover_model_name_list, cal_ratio_list, model_CE_loss_cal,
                                            model_CE_loss_backbone, ce_lis_cal, ce_lis_backbone,
                                            model_BN_loss, loss_BN_lis, model_total_loss, weight_list,
                                            model_FC_loss, fc_lis, model_SC_loss, SC_lis)
                draw_loss_cal(targets[0].item(), num_call, args, recover_model_name_list, cal_ratio_list,
                              model_CE_loss_cal, model_CE_loss_backbone, model_BN_loss, model_total_loss,
    ...
    for model_name in recover_model_name_list:
        last_feature_hooks[model_name].close()
...
def parse_args():
    ...
    parser.add_argument('--selected_size', type=int, default=2,
                        help='number of recover models to optimise the synthetic data')
    parser.add_argument('--voter_type', type=str, default='prior', choices=['equal', 'random', 'prior'],
                        help='The voter type, Equal assigns equal weight, '
                             'Random assigns random weight and Prior assigns weight using prior information')
    ...
    parser.add_argument('--FC', action='store_true', help='whether to use Fine-grained characteristic Constraint')
    parser.add_argument('--FC_ratio', type=float, default=0.8, help='the ratio of FC')
    parser.add_argument('--IntraFC_ratio', type=float, default=0.5, help='the ratio of intra_fc')
    ...
    if args.dataset_name == 'SC_imsize64':
        ...
        args.model_prior_weight_dict = {'ResNet18': None, 'ResNet50': None, 'ShuffleNetV2': None, 'MobileNetV2': None,
                                        'Densenet121': None}
        ...
        args.model_prior_weight_dict = {'ResNet18': 70.2, 'ResNet50': 73.4, 'ShuffleNetV2': None, 'MobileNetV2': 69.6,
                                        'Densenet121': 76.6}
