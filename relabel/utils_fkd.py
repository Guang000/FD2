...
class RandomResizedCropWithCoords(torchvision.transforms.RandomResizedCrop):
    ...
    def __call__(self, img, coords):
        ...
        return t_F.resized_crop(img, i, j, h, w, self.size, self.interpolation), coords
...
class RandomHorizontalFlipWithRes(torch.nn.Module):
    ...
    def forward(self, img, status):
        ...
        Args:
            ...
            status (True/False/None): If True, this image will be flipped; if False, this image will not be flipped;
                                      if None, this image will be flipped with a 50% probability.
        Returns:
            PIL Image or Tensor: Randomly flipped image.
            bool: True if this image is be flipped, False if this image isn't be flipped
        """
...
def get_FKD_info(fkd_path):
    ...
    batch_list = sorted(os.listdir(os.path.join(fkd_path, 'epoch_0')), key=custom_sort_key)
    batch_size = torch.load(os.path.join(fkd_path, 'epoch_0', batch_list[0]), weights_only=False)[1].size()[0]
    last_batch_size = torch.load(os.path.join(fkd_path, 'epoch_0', batch_list[-1]))[1].size()[0]
...
class ImageFolder_FKD_MIX(torchvision.datasets.ImageFolder):
    def __init__(self, fkd_path, mode, args_epoch=None, args_bs=None, **kwargs):
        ...
        if self.mode == 'fkd_load':
            ...
            if args_bs != batch_size:
                raise ValueError('`--batch_size` should be same in both saving and loading phase. Please use '
                                 '`--gradient_accumulation_steps` to control batch size in model forward phase.')
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            sample_new, flip_status, coords_status = sample, None, None
...
def load_model(args, model_name):
    ...
    state_dict = torch.load(str(model_weight_path), weights_only=True, map_location="cpu")
    model = None

    def pruning_classifier(model=None, classes=None):
        if classes is None:
            classes = []
        try:
            model_named_parameters = [name for name, x in model.named_parameters()]
            for name, x in model.named_parameters():
                if name == model_named_parameters[-1] or name == model_named_parameters[-2]:
                    x.data = x[classes]
        except:
    ...
    if 'conv' in model_name:
        ...
        try:
            model = utils_models.load_model(model_name, args.ncls, "CVDD", False)
            model.load_state_dict(state_dict)
        except RuntimeError:
            print(f"CVDD's {model_name} can't match ckpt, next try torchvision")
            try:
                model = utils_models.load_model(model_name, args.ncls, "torchvision", False)
                model.load_state_dict(state_dict)
...
def load_model_cal(args, model_id, model_name):
    model_weight_path = os.path.join(args.model_pool_dir, model_name + f'_M{args.M[model_id]}_{to_sci_text(args.cal_ratio[model_id])}cal.pth')
    state_dict = torch.load(str(model_weight_path), weights_only=True, map_location="cpu")
    model, cal, source = None, None, None
    ...
    try:
        ...
        try:
            ...
            print(f"torchvision's {model_name} can match ckpt")
            source = "torchvision"
            cal = CAL(args.ncls, args.M[model_id], model_name, "torchvision")
    else:
        print(f"CVDD's {model_name} can match ckpt")
        source = "CVDD"
        cal = CAL(args.ncls, args.M[model_id], model_name, "CVDD")

    cal.load_state_dict(state_dict["cal"])

    return model, cal, source
